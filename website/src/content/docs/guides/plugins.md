---
title: Plugins
description: Package and distribute tools as reusable toolkits.
---
...
:::note
The entry point value should point to the class itself, not an instance. `PluginManager` calls `entry_point.load()`, which returns the class, then passes it to `register()`.
:::
