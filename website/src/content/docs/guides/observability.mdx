...
`OTelHooks.console()` sets up a `TracerProvider` and `ConsoleSpanExporter` automatically — no boilerplate required. Call `hooks.flush()` after `agent.run()` if spans appear truncated.
...
hooks = OTelHooks.otlp("http://localhost:4317")
