---
title: MCP
description: Connect to MCP servers or publish your tools as one.
---

The [Model Context Protocol](https://modelcontextprotocol.io) (MCP) is an open standard for exposing tools to LLMs. Cyclops ships a lightweight client and server so you can consume tools from any MCP server or publish your own Cyclops tools as an MCP server.
...
The [MCP filesystem server](https://github.com/modelcontextprotocol/servers/tree/main/src/filesystem) is a widely used MCP server that exposes file-system operations. Connect to it with npx:
